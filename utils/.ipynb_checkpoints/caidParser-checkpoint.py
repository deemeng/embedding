...
>disprot_ID
sequence
reference (2 classes: 0, 1), '1' means linker.
--------------
...
def get_labelInfo(seq_label: list):
    ...
    for i, label in enumerate(seq_label):
        ...
        if ((label==0) or i==len(seq_label)-1) and (count_len>0): # the end of a region
    ...
    with open(file_path, 'r') as file:
        ...
        for line in file:
            if line_counter%3==0:
                ...
                dict_seq['reference'] = [int(r) for r in line.strip()]
                # get linker information: start, end, length, num_linkers
                dict_seq['info_linker'], dict_seq['num_linker'] = get_labelInfo(dict_seq['reference'])
